...
class GetHabitaciones extends AbstractHelper implements ServiceLocatorAwareInterface {
    ...
    public function __invoke($idPabellon = 0) {
        ...
        try {
            if($idPabellon != 0) {
                $ret = $table->findByPabellon($idPabellon);
            } else {
                $ret = $table->fetchAll();
            }
        } catch (\Exception $e) {
